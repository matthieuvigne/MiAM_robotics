...
void ServoHandler::moveArm(bool const& rightArm, arm const& pose)
{
    if (rightArm)
    {
        switch (pose)
        {
            case arm::MEASURE: maestro_->setPosition(RIGHT_ARM, 1620); break;
...
void ServoHandler::moveSuction(int const& suctionNumber, suction const& position)
{
    ...
    else if (suctionNumber == 1)
    {
        switch (position)
        {
            ...
            case suction::DROP_FAKE_STATUE: maestro_->setPosition(SERVO_SUCTION[1], 1180); break;
            case suction::VERTICAL:  maestro_->setPosition(SERVO_SUCTION[1], 1500); break;
            default: break;
        }
    }
    else
