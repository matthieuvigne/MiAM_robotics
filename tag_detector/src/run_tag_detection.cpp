...
// ________________________________________ Camera2 _________________________________________________
...
class Camera2 {
  ...
  public:
    Camera2() = delete;
    Camera2(int width, int height);
    Camera2(int width, int height, double fx, double fy, double cx, double cy);
    typedef std::shared_ptr<Camera2> Ptr;
    typedef std::unique_ptr<Camera2> UniquePtr;
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    virtual ~Camera2() = default;
...
Camera2::Camera2(int width, int height)
  ...
  cx_(width/2.), cy_(height/2.)
{
  ...
  // Configure the internal Camera2 object
  std::lock_guard<std::mutex> lock(mutex_);
  is_running_on_RPi_ =
    !camera_.initCamera(width_,height_,libcamera::formats::RGB888,1,0);
  if(is_running_on_RPi_)
  {
    ...
    //controls.set(libcamera::controls::Contrast, 1.0);
...
Camera2::Camera2(int width, int height, double fx, double fy, double cx, double cy)
: Camera2(width, height)
{
  // Set focal lenghts
  assert( (fx>0.) && (fy>0.) );
  fx_ = fx;
...
cv::Mat Camera2::get_camera_matrix() const
{
...
bool Camera2::take_picture(cv::Mat* image_ptr, double timeout)
{
  ...
  // Otherwise, get the image from the Camera2
  bool success = false;
  ...
  if(success)
  {
    image = cv::Mat(height_,width_,CV_8UC3,frame_data_.imageData).clone();
  }
  else
  {
    frame_data_.size = 0;
...
bool Camera2::get_markers(MarkerList* markers_ptr)
{
...
int main(int argc, char* argv[])
{
  ...
  int const cx = fx/2.;
  int const cy = fy/2.;
  Camera2::Ptr camera_ptr(new Camera2(width,height,fx,fy,cx,cy));
