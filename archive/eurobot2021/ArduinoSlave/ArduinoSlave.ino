...
// Reading resistor values on A1 and A2
int const EXPECTED_RESISTOR[3] = {411, 631, 903};
#define RESISTOR_TOLERANCE 100
...
int resistorOneValue = 0;
int resistorTwoValue = 0;
int resistorRegister = 0;
void loop() {
 // Read potentiometer position.
 potentiometerPosition = analogRead(A0);
 resistorOneValue = analogRead(A1);
 resistorTwoValue = analogRead(A2);
 resistorRegister = 0;
 for (int i = 0; i < 3; i++)
 {
  if (abs(resistorOneValue - EXPECTED_RESISTOR[i]) < RESISTOR_TOLERANCE)
  {
    resistorRegister |= i + 1;
  }
  if (abs(resistorTwoValue - EXPECTED_RESISTOR[i]) < RESISTOR_TOLERANCE)
  {
    resistorRegister |= (i + 1) << 2;
  }
 }
 ...
 Serial.write(resistorRegister & 0xFF);
 checksum += resistorRegister & 0xFF;
