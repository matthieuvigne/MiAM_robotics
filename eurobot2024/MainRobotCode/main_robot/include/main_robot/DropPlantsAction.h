...
class DropPlantsWithPotAction: public AbstractAction
{
public:
    /// @brief Action to pick up plants from a given zone.
    /// @param zoneCenter Center of the zone from which to pick-up the plants
    DropPlantsWithPotAction(RobotInterface *robot, ServoManager *servoManager, int const& zoneId):
...
class DropPlantsWithoutPotsAction: public AbstractAction
{
public:
    /// @brief Action to pick up plants from a given zone.
    /// @param zoneCenter Center of the zone from which to pick-up the plants
    DropPlantsWithoutPotsAction(RobotInterface *robot, ServoManager *servoManager, int const& zoneId):
        AbstractAction("Drop plants (no pots) " + std::to_string(zoneId), robot),
        servoManager_(servoManager),
        zoneId_(zoneId)
    {}
