...
MotionController::MotionController(RobotParameters const &robotParameters, bool const& disableEncoders) : 
                                                                             currentPosition_(),
                                                                             newTrajectories_(),
                                                           ...
                                                           bool const &hasMatchStarted)
{
    ...
    if (!disableEncoders_)
    {
        currentPosition = currentPosition_.update(kinematics_, measurements.encoderSpeed);
        baseSpeed = kinematics_.forwardKinematics(measurements.encoderSpeed, true);
    }
    else if (!currentTrajectories_.empty())
    {
        TrajectoryPoint currentTrajectoryPoint = currentTrajectories_.front()->getCurrentPoint(curvilinearAbscissa_);
        currentPosition = currentTrajectoryPoint.position;
        currentPosition_.set(currentPosition);
        baseSpeed = {currentTrajectoryPoint.linearVelocity, currentTrajectoryPoint.angularVelocity};
    }
