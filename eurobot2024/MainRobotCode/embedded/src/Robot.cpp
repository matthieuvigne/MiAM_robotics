...
Robot::Robot(RobotParameters const& parameters, AbstractStrategy *strategy, RobotGUI *gui, bool const& testMode, bool const& disableLidar, bool const& disableEncoders):
    RobotInterface(parameters),
    gui_(gui),
    lidar_(parameters.lidarOffset),
    testMode_(testMode),
    spiStepperMotors_(RPI_SPI_00, 1000000),
    stepperMotors_(&spiStepperMotors_, 2),
    disableLidar_(disableLidar),
    disableEncoders_(disableEncoders),
    spiEncoder_(RPI_SPI_01, 1000000),
    encoders_(&spiEncoder_, 2),
    strategy_(strategy)
{
    motionController_.init(RobotPosition());
    motionController_.disableEncoders_ = disableEncoders_;

    guiState_.state = robotstate::INIT;
    guiState_.score = 0;

    lastEncoderPosition_ = {0, 0};
}
...
bool Robot::initSystem()
{
    ...
    if (!isStepperInit_)
    {
        ...
        int maxSpeed = motionController_.robotParams_.maxWheelSpeed / motionController_.robotParams_.wheelRadius / stepSize;
        int maxAcceleration = motionController_.robotParams_.maxWheelAcceleration / motionController_.robotParams_.wheelRadius / stepSize;
        ...
        if (!isStepperInit_)
        {
            guiState_.debugStatus += "Stepper init failed\n";
            // #ifdef DEBUG
    ...
    // Encoder init
    if (disableEncoders_)
        std::cout << "Encoders disabled" << std::endl;
    if (!isEncodersInit_ && !disableEncoders_)
    {
    ...
    return isStepperInit_ & (isLidarInit_ || disableLidar_); // & isEncodersInit_ & isServoInit_
}
...
bool Robot::setupBeforeMatchStart()
{
    ...
    if (guiState_.state == robotstate::INIT)
    {
        ...
        if (isStepperInit_)
        {
            ...
            // Test motors.
            std::vector<double> motorSpeed_({0, 0});
            motorSpeed_[0] = 150;
            motorSpeed_[1] = 150;
            ...
            motorSpeed_[1] = 0;
            stepperMotors_.setSpeed(motorSpeed_);
            std::cout << "End testing motors" << std::endl;
        }
    }
...
void Robot::lowLevelLoop()
{
    ...
    while(!hasMatchStarted_ || (currentTime_ < 100.0 + matchStartTime_))
    {
        ...
        if (!disableEncoders_)
        {
            encoderPosition = encoders_.updatePosition();
        }
        else
        {
            encoderPosition = {0, 0};
        ...
        else
        {
            ...
            // {
                ...
                measurements.motorSpeed(0) = target.motorSpeed[0];
                measurements.motorSpeed(1) = target.motorSpeed[1];
                ...
                // std::cout << "target.motorSpeed " << target.motorSpeed << std::endl;
                stepperMotors_.setSpeed({target.motorSpeed[0] / stepSize, target.motorSpeed[1] / stepSize});
