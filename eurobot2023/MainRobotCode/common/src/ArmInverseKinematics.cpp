  ...
  std::array<double,4>* angles_ptr)
{
  ...
  double const psi0_rad = std::atan(z0/r0);
  ...
  {    
    ...
    double constexpr theta23_max = 0;
    double constexpr theta23_min = - M_PI_2;
    bool const ok1 = std::max(std::fabs(r_error),std::fabs(z_error))<1e-10;
