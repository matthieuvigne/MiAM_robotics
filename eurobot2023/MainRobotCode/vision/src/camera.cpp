  ...
  std::string const& imageLogPath) const
{
  ...
  //~ Eigen::Affine3d const TRC = common::getTRC(camera_azimuth_deg, camera_elevation_deg); [REMOVE]
  ...
  for(int marker_idx=0; marker_idx<num_markers; ++marker_idx)
  {
    ...
    MarkerId const marker_id = static_cast<MarkerId>(detected_marker_ids[marker_idx]);
    Marker::UniquePtr marker_ptr(new Marker(marker_id));
    ...
    for(int corner_idx=0; corner_idx<num_corners; ++corner_idx)
    {
      ...
      Eigen::Matrix<double,3,6> J_CpCi_TCM = common::poseJacobian(TCM, MpCi);
    ...
    // Add the measurement
    marker_ptr->addMeasurement(timestamp_ns, TCM, cov_TCM);
...
Camera::Params Camera::Params::getDefaultParams()
{
  ...
  params.name = "camera";
  params.resolution[Camera::Params::WIDTH]  = 1280;
  params.resolution[Camera::Params::HEIGHT] = 960;
  params.intrinsics[Camera::Params::FX]     = 1368.818;
  params.intrinsics[Camera::Params::FY]     = 1358.929;
  params.intrinsics[Camera::Params::CX]     = 542.308;
  params.intrinsics[Camera::Params::CY]     = 476.351;
  params.distortion_model = DistortionModel::Type::NoDistortion;
